...
test.describe('Radiologist Portal Authentication Suite - Complete Valid & Invalid Validation Scenarios', () => {
  ...
  // ==========================================
  test('TC_RADIOLOGIST_001 [VALID]: Successful Radiologist Login & Role Switch with Navigation Menu Verification', async ({ page }) => {
    ...
    const roleCard = page.locator('.MuiCard-root, [class*="card"], div')
      .filter({ hasText: new RegExp('^' + 'Radiologist' + '$', 'i') })
      .or(page.getByText('Radiologist', { exact: true }))
      .first();
  ...
  // ==========================================
  test('TC_RADIOLOGIST_002 [INVALID]: Radiologist Login with Wrong Password', async ({ page }) => {
  ...
  test('TC_RADIOLOGIST_003 [INVALID]: Radiologist Login with Non-Existent Username', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.wrongEmailUser.username, testData.invalidUser.wrongEmailUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_RADIOLOGIST_004 [INVALID]: Radiologist Submit Empty Credentials', async ({ page }) => {
    const staffLoginBtn = page.getByRole('button', { name: 'Staff Login' }).first();
    if (await staffLoginBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await staffLoginBtn.click({ force: true });
    }
  ...
  test('TC_RADIOLOGIST_005 [INVALID]: Radiologist Email Format Validation - Missing @ Symbol', async ({ page }) => {
  ...
  test('TC_RADIOLOGIST_006 [INVALID]: Radiologist Email Format Validation - Missing Domain Extension', async ({ page }) => {
  ...
  test('TC_RADIOLOGIST_007 [INVALID]: Radiologist Empty Password Validation', async ({ page }) => {
    await performStaffLogin(page, testData.adminUser.username, '');

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_RADIOLOGIST_008 [INVALID]: Radiologist Empty Email Validation', async ({ page }) => {
    await performStaffLogin(page, '', testData.adminUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_RADIOLOGIST_009 [INVALID]: Radiologist Security Validation - SQL Injection attempt', async ({ page }) => {
  ...
  test('TC_RADIOLOGIST_010 [VALIDATION]: Radiologist Whitespace Auto-Trim Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.whitespaceEmail.username, testData.invalidUser.whitespaceEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_RADIOLOGIST_011 [VALIDATION]: Radiologist Email Case-Insensitivity Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.uppercaseEmail.username, testData.invalidUser.uppercaseEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_RADIOLOGIST_012 [SECURITY]: Radiologist XSS Payload Security Check', async ({ page }) => {
