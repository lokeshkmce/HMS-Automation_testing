...
test.describe('Nurse Portal Authentication Suite - Complete Valid & Invalid Validation Scenarios', () => {
  ...
  // ==========================================
  test('TC_NURSE_001 [VALID]: Successful Nurse Login & Role Switch with Navigation Menu Verification', async ({ page }) => {
    ...
    const roleCard = page.locator('.MuiCard-root, [class*="card"], div')
      .filter({ hasText: new RegExp('^' + 'Nurse' + '$', 'i') })
      .or(page.getByText('Nurse', { exact: true }))
      .first();
    ...
    await expect(page).toHaveURL(/.*(nurse|staff|triage)/i);
  });

  // ==========================================
  // 2. INVALID & FORM VALIDATION TEST CASES
  // ==========================================
  test('TC_NURSE_002 [INVALID]: Nurse Login with Wrong Password', async ({ page }) => {
  ...
  test('TC_NURSE_003 [INVALID]: Nurse Login with Non-Existent Username', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.wrongEmailUser.username, testData.invalidUser.wrongEmailUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_NURSE_004 [INVALID]: Nurse Submit Empty Credentials', async ({ page }) => {
    const staffLoginBtn = page.getByRole('button', { name: 'Staff Login' }).first();
    if (await staffLoginBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await staffLoginBtn.click({ force: true });
    }
  ...
  test('TC_NURSE_005 [INVALID]: Nurse Email Format Validation - Missing @ Symbol', async ({ page }) => {
  ...
  test('TC_NURSE_006 [INVALID]: Nurse Email Format Validation - Missing Domain Extension', async ({ page }) => {
  ...
  test('TC_NURSE_007 [INVALID]: Nurse Empty Password Validation', async ({ page }) => {
    await performStaffLogin(page, testData.adminUser.username, '');

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_NURSE_008 [INVALID]: Nurse Empty Email Validation', async ({ page }) => {
    await performStaffLogin(page, '', testData.adminUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_NURSE_009 [INVALID]: Nurse Security Validation - SQL Injection attempt', async ({ page }) => {
