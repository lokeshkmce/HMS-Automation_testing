...
test.describe('Admin Portal Authentication Suite - Complete Valid & Invalid Validation Scenarios', () => {
  ...
  // ==========================================
  test('TC_ADMIN_001 [VALID]: Successful Admin Login & Role Switch with Navigation Menu Verification', async ({ page }) => {
    ...
    const roleCard = page.locator('.MuiCard-root, [class*="card"], div')
      .filter({ hasText: new RegExp('^' + 'Admin' + '$', 'i') })
    ...
    await expect(page).toHaveURL(/.*(admin|staff|dashboard)/i);
  });

  // ==========================================
  // 2. INVALID & FORM VALIDATION TEST CASES
  // ==========================================
  test('TC_ADMIN_002 [INVALID]: Admin Login with Wrong Password', async ({ page }) => {
  ...
  test('TC_ADMIN_003 [INVALID]: Admin Login with Non-Existent Username', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.wrongEmailUser.username, testData.invalidUser.wrongEmailUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_ADMIN_004 [INVALID]: Admin Submit Empty Credentials', async ({ page }) => {
    const staffLoginBtn = page.getByRole('button', { name: 'Staff Login' }).first();
    if (await staffLoginBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await staffLoginBtn.click({ force: true });
    }
  ...
  test('TC_ADMIN_005 [INVALID]: Admin Email Format Validation - Missing @ Symbol', async ({ page }) => {
  ...
  test('TC_ADMIN_006 [INVALID]: Admin Email Format Validation - Missing Domain Extension', async ({ page }) => {
  ...
  test('TC_ADMIN_007 [INVALID]: Admin Empty Password Validation', async ({ page }) => {
    await performStaffLogin(page, testData.adminUser.username, '');

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_ADMIN_008 [INVALID]: Admin Empty Email Validation', async ({ page }) => {
    await performStaffLogin(page, '', testData.adminUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_ADMIN_009 [INVALID]: Admin Security Validation - SQL Injection attempt', async ({ page }) => {
