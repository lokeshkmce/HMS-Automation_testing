...
test.describe('Group Admin Portal Authentication Suite - Complete Valid & Invalid Validation Scenarios', () => {
  ...
  // ==========================================
  test('TC_GROUP_ADMIN_001 [VALID]: Successful Group Admin Login & Role Switch with Navigation Menu Verification', async ({ page }) => {
    ...
    const roleCard = page.locator('.MuiCard-root, [class*="card"], div')
      .filter({ hasText: new RegExp('^' + 'Group Admin' + '$', 'i') })
      .or(page.getByText('Group Admin', { exact: true }))
      .first();
  ...
  // ==========================================
  test('TC_GROUP_ADMIN_002 [INVALID]: Group Admin Login with Wrong Password', async ({ page }) => {
  ...
  test('TC_GROUP_ADMIN_003 [INVALID]: Group Admin Login with Non-Existent Username', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.wrongEmailUser.username, testData.invalidUser.wrongEmailUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_GROUP_ADMIN_004 [INVALID]: Group Admin Submit Empty Credentials', async ({ page }) => {
    const staffLoginBtn = page.getByRole('button', { name: 'Staff Login' }).first();
    if (await staffLoginBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await staffLoginBtn.click({ force: true });
    }
  ...
  test('TC_GROUP_ADMIN_005 [INVALID]: Group Admin Email Format Validation - Missing @ Symbol', async ({ page }) => {
  ...
  test('TC_GROUP_ADMIN_006 [INVALID]: Group Admin Email Format Validation - Missing Domain Extension', async ({ page }) => {
  ...
  test('TC_GROUP_ADMIN_007 [INVALID]: Group Admin Empty Password Validation', async ({ page }) => {
    await performStaffLogin(page, testData.adminUser.username, '');

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_GROUP_ADMIN_008 [INVALID]: Group Admin Empty Email Validation', async ({ page }) => {
    await performStaffLogin(page, '', testData.adminUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_GROUP_ADMIN_009 [INVALID]: Group Admin Security Validation - SQL Injection attempt', async ({ page }) => {
  ...
  test('TC_GROUP_ADMIN_010 [VALIDATION]: Group Admin Whitespace Auto-Trim Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.whitespaceEmail.username, testData.invalidUser.whitespaceEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_GROUP_ADMIN_011 [VALIDATION]: Group Admin Email Case-Insensitivity Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.uppercaseEmail.username, testData.invalidUser.uppercaseEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_GROUP_ADMIN_012 [SECURITY]: Group Admin XSS Payload Security Check', async ({ page }) => {
