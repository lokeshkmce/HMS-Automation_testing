...
test.describe('Pharmacist Portal Authentication Suite - Complete Valid & Invalid Validation Scenarios', () => {
  ...
  // ==========================================
  test('TC_PHARMACIST_001 [VALID]: Successful Pharmacist Login & Role Switch with Navigation Menu Verification', async ({ page }) => {
    ...
    const roleCard = page.locator('.MuiCard-root, [class*="card"], div')
      .filter({ hasText: new RegExp('^' + 'Pharmacist' + '$', 'i') })
      .or(page.getByText('Pharmacist', { exact: true }))
      .first();
    ...
    await expect(page).toHaveURL(/.*(pharmacy|staff|dispense)/i);
  });

  // ==========================================
  // 2. INVALID & FORM VALIDATION TEST CASES
  // ==========================================
  test('TC_PHARMACIST_002 [INVALID]: Pharmacist Login with Wrong Password', async ({ page }) => {
  ...
  test('TC_PHARMACIST_003 [INVALID]: Pharmacist Login with Non-Existent Username', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.wrongEmailUser.username, testData.invalidUser.wrongEmailUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_PHARMACIST_004 [INVALID]: Pharmacist Submit Empty Credentials', async ({ page }) => {
    const staffLoginBtn = page.getByRole('button', { name: 'Staff Login' }).first();
    if (await staffLoginBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await staffLoginBtn.click({ force: true });
    }
  ...
  test('TC_PHARMACIST_005 [INVALID]: Pharmacist Email Format Validation - Missing @ Symbol', async ({ page }) => {
  ...
  test('TC_PHARMACIST_006 [INVALID]: Pharmacist Email Format Validation - Missing Domain Extension', async ({ page }) => {
  ...
  test('TC_PHARMACIST_007 [INVALID]: Pharmacist Empty Password Validation', async ({ page }) => {
    await performStaffLogin(page, testData.adminUser.username, '');

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_PHARMACIST_008 [INVALID]: Pharmacist Empty Email Validation', async ({ page }) => {
    await performStaffLogin(page, '', testData.adminUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_PHARMACIST_009 [INVALID]: Pharmacist Security Validation - SQL Injection attempt', async ({ page }) => {
  ...
  test('TC_PHARMACIST_010 [VALIDATION]: Pharmacist Whitespace Auto-Trim Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.whitespaceEmail.username, testData.invalidUser.whitespaceEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_PHARMACIST_011 [VALIDATION]: Pharmacist Email Case-Insensitivity Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.uppercaseEmail.username, testData.invalidUser.uppercaseEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_PHARMACIST_012 [SECURITY]: Pharmacist XSS Payload Security Check', async ({ page }) => {
