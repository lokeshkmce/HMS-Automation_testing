...
test.describe('Billing Clerk Portal Authentication Suite - Complete Valid & Invalid Validation Scenarios', () => {
  ...
  // ==========================================
  test('TC_BILLING_001 [VALID]: Successful Billing Clerk Login & Role Switch with Navigation Menu Verification', async ({ page }) => {
    ...
    const roleCard = page.locator('.MuiCard-root, [class*="card"], div')
      .filter({ hasText: new RegExp('^' + 'Billing Clerk' + '$', 'i') })
      .or(page.getByText('Billing Clerk', { exact: true }))
      .first();
    ...
    await expect(page).toHaveURL(/.*(billing|staff|invoices)/i);
  });

  // ==========================================
  // 2. INVALID & FORM VALIDATION TEST CASES
  // ==========================================
  test('TC_BILLING_002 [INVALID]: Billing Clerk Login with Wrong Password', async ({ page }) => {
  ...
  test('TC_BILLING_003 [INVALID]: Billing Clerk Login with Non-Existent Username', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.wrongEmailUser.username, testData.invalidUser.wrongEmailUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_BILLING_004 [INVALID]: Billing Clerk Submit Empty Credentials', async ({ page }) => {
    const staffLoginBtn = page.getByRole('button', { name: 'Staff Login' }).first();
    if (await staffLoginBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await staffLoginBtn.click({ force: true });
    }
  ...
  test('TC_BILLING_005 [INVALID]: Billing Clerk Email Format Validation - Missing @ Symbol', async ({ page }) => {
  ...
  test('TC_BILLING_006 [INVALID]: Billing Clerk Email Format Validation - Missing Domain Extension', async ({ page }) => {
  ...
  test('TC_BILLING_007 [INVALID]: Billing Clerk Empty Password Validation', async ({ page }) => {
    await performStaffLogin(page, testData.adminUser.username, '');

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_BILLING_008 [INVALID]: Billing Clerk Empty Email Validation', async ({ page }) => {
    await performStaffLogin(page, '', testData.adminUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_BILLING_009 [INVALID]: Billing Clerk Security Validation - SQL Injection attempt', async ({ page }) => {
  ...
  test('TC_BILLING_010 [VALIDATION]: Billing Clerk Whitespace Auto-Trim Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.whitespaceEmail.username, testData.invalidUser.whitespaceEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_BILLING_011 [VALIDATION]: Billing Clerk Email Case-Insensitivity Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.uppercaseEmail.username, testData.invalidUser.uppercaseEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_BILLING_012 [SECURITY]: Billing Clerk XSS Payload Security Check', async ({ page }) => {
