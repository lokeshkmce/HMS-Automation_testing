...
test.describe('Receptionist Portal Authentication Suite - Complete Valid & Invalid Validation Scenarios', () => {
  ...
  // ==========================================
  test('TC_RECEPTIONIST_001 [VALID]: Successful Receptionist Login & Role Switch with Navigation Menu Verification', async ({ page }) => {
    ...
    const roleCard = page.locator('.MuiCard-root, [class*="card"], div')
      .filter({ hasText: new RegExp('^' + 'Receptionist' + '$', 'i') })
    ...
    await expect(page).toHaveURL(/.*(receptionist|staff|check-in)/i);
  });

  // ==========================================
  // 2. INVALID & FORM VALIDATION TEST CASES
  // ==========================================
  test('TC_RECEPTIONIST_002 [INVALID]: Receptionist Login with Wrong Password', async ({ page }) => {
  ...
  test('TC_RECEPTIONIST_003 [INVALID]: Receptionist Login with Non-Existent Username', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.wrongEmailUser.username, testData.invalidUser.wrongEmailUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_RECEPTIONIST_004 [INVALID]: Receptionist Submit Empty Credentials', async ({ page }) => {
    const staffLoginBtn = page.getByRole('button', { name: 'Staff Login' }).first();
    if (await staffLoginBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await staffLoginBtn.click({ force: true });
    }
  ...
  test('TC_RECEPTIONIST_005 [INVALID]: Receptionist Email Format Validation - Missing @ Symbol', async ({ page }) => {
  ...
  test('TC_RECEPTIONIST_006 [INVALID]: Receptionist Email Format Validation - Missing Domain Extension', async ({ page }) => {
  ...
  test('TC_RECEPTIONIST_007 [INVALID]: Receptionist Empty Password Validation', async ({ page }) => {
    await performStaffLogin(page, testData.adminUser.username, '');

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_RECEPTIONIST_008 [INVALID]: Receptionist Empty Email Validation', async ({ page }) => {
    await performStaffLogin(page, '', testData.adminUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_RECEPTIONIST_009 [INVALID]: Receptionist Security Validation - SQL Injection attempt', async ({ page }) => {
  ...
  test('TC_RECEPTIONIST_010 [VALIDATION]: Receptionist Whitespace Auto-Trim Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.whitespaceEmail.username, testData.invalidUser.whitespaceEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_RECEPTIONIST_011 [VALIDATION]: Receptionist Email Case-Insensitivity Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.uppercaseEmail.username, testData.invalidUser.uppercaseEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_RECEPTIONIST_012 [SECURITY]: Receptionist XSS Payload Security Check', async ({ page }) => {
