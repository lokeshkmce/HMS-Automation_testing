...
test.describe('Org Admin Portal Authentication Suite - Complete Valid & Invalid Validation Scenarios', () => {
  ...
  // ==========================================
  test('TC_ORG_ADMIN_001 [VALID]: Successful Org Admin Login & Role Switch with Navigation Menu Verification', async ({ page }) => {
    ...
    const roleCard = page.locator('.MuiCard-root, [class*="card"], div')
      .filter({ hasText: new RegExp('^' + 'Org Admin' + '$', 'i') })
      .or(page.getByText('Org Admin', { exact: true }))
      .first();
  ...
  // ==========================================
  test('TC_ORG_ADMIN_002 [INVALID]: Org Admin Login with Wrong Password', async ({ page }) => {
  ...
  test('TC_ORG_ADMIN_003 [INVALID]: Org Admin Login with Non-Existent Username', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.wrongEmailUser.username, testData.invalidUser.wrongEmailUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_ORG_ADMIN_004 [INVALID]: Org Admin Submit Empty Credentials', async ({ page }) => {
    const staffLoginBtn = page.getByRole('button', { name: 'Staff Login' }).first();
    if (await staffLoginBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await staffLoginBtn.click({ force: true });
    }
  ...
  test('TC_ORG_ADMIN_005 [INVALID]: Org Admin Email Format Validation - Missing @ Symbol', async ({ page }) => {
  ...
  test('TC_ORG_ADMIN_006 [INVALID]: Org Admin Email Format Validation - Missing Domain Extension', async ({ page }) => {
  ...
  test('TC_ORG_ADMIN_007 [INVALID]: Org Admin Empty Password Validation', async ({ page }) => {
    await performStaffLogin(page, testData.adminUser.username, '');

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_ORG_ADMIN_008 [INVALID]: Org Admin Empty Email Validation', async ({ page }) => {
    await performStaffLogin(page, '', testData.adminUser.password);

    await expect(page).not.toHaveURL(/.*\/staff\/select-role/);
  });

  test('TC_ORG_ADMIN_009 [INVALID]: Org Admin Security Validation - SQL Injection attempt', async ({ page }) => {
  ...
  test('TC_ORG_ADMIN_010 [VALIDATION]: Org Admin Whitespace Auto-Trim Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.whitespaceEmail.username, testData.invalidUser.whitespaceEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_ORG_ADMIN_011 [VALIDATION]: Org Admin Email Case-Insensitivity Check', async ({ page }) => {
    await performStaffLogin(page, testData.invalidUser.uppercaseEmail.username, testData.invalidUser.uppercaseEmail.password);

    await page.waitForTimeout(2000);
  });

  test('TC_ORG_ADMIN_012 [SECURITY]: Org Admin XSS Payload Security Check', async ({ page }) => {
