...
test.describe('Step 1: Patient Appointment Booking - ENT', () => {
  ...
  test('TC_APPT_ENT [VALID]: Patient Appointment Booking for ENT', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'ENT'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'ENT', exact: true })
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA ent', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'ent', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA ent" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA ent" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine ENT checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
