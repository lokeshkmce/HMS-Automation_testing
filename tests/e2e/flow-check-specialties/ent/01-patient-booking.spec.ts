...
test.describe('Step 1: Patient Appointment Booking - ENT', () => {
  ...
  test('TC_APPT_ENT [VALID]: Patient Appointment Booking for ENT', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA ent', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA ent' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'ENT' }))
      .first();
