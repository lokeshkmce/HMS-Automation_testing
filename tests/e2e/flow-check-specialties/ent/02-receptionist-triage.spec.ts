...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - ENT', () => {
  ...
  test('TC_TRIAGE_ENT [VALID]: Receptionist Check-In & Triage for ENT', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.ent@omnivva.com');
    ...
    await expect(docOption, `QA Doctor "Dr. QA ent" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine ENT checkup and nurse triage evaluation');
