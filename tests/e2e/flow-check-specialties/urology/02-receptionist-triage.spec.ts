...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Urology', () => {

  test('TC_TRIAGE_UROLOGY [VALID]: Receptionist Check-In & Triage for Urology', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.urology@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Urology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Urology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA urology' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA urology', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'urology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA urology" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Urology checkup and nurse triage evaluation');
