  ...
  test('TC_TRIAGE_UROLOGY [VALID]: Receptionist Check-In & Triage for Urology', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.urology@omnivva.com',
        'qa.urology@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.urology@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.urology@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.urology@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA urology', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Urology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Urology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Urology checkup');
    }
