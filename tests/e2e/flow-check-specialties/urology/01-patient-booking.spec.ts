...
test.describe('Step 1: Patient Appointment Booking - Urology', () => {
  ...
  test('TC_APPT_UROLOGY [VALID]: Patient Appointment Booking for Urology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA urology', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA urology' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Urology' }))
      .first();
