...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Oncology', () => {
  ...
  test('TC_TRIAGE_ONCOLOGY [VALID]: Receptionist Check-In & Triage for Oncology', async ({ page }) => {
    ...
    const emailsToTry = Array.from(new Set([
      'qa.oncology@omnivva.com',
      'qa.oncology@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
      'qa.oncology@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
      'qa.oncology@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
      'qa.oncology@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
    ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA oncology', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Oncology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Oncology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Oncology checkup');
    }
