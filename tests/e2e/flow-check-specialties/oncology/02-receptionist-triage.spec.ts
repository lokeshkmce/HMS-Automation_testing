...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Oncology', () => {

  test('TC_TRIAGE_ONCOLOGY [VALID]: Receptionist Check-In & Triage for Oncology', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.oncology@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Oncology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Oncology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA oncology' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA oncology', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'oncology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA oncology" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Oncology checkup and nurse triage evaluation');
