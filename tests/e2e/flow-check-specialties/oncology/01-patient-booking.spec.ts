...
test.describe('Step 1: Patient Appointment Booking - Oncology', () => {
  ...
  test('TC_APPT_ONCOLOGY [VALID]: Patient Appointment Booking for Oncology', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Oncology'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Oncology', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Oncology' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA oncology', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'oncology', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA oncology" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA oncology" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Oncology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
