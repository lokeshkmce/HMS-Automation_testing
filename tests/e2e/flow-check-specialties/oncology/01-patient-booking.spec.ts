...
test.describe('Step 1: Patient Appointment Booking - Oncology', () => {
  ...
  test('TC_APPT_ONCOLOGY [VALID]: Patient Appointment Booking for Oncology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA oncology', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA oncology' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Oncology' }))
      .first();
