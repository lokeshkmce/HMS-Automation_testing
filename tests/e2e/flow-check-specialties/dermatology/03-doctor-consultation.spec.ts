...
test.describe('Step 3: Doctor Consultation & Prescription - Dermatology', () => {
  ...
  test('TC_DOC_DERMATOLOGY [VALID]: Doctor Consultation for Dermatology', async ({ page }) => {
    ...
    if (await provisionalDiagnosisInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await provisionalDiagnosisInput.fill('Diagnosis - Dermatology');
    }
    ...
    if (await writePrescriptionBtn.isVisible({ timeout: 4000 }).catch(() => false)) {
      ...
      if (await medicineSearchInput.isVisible({ timeout: 4000 }).catch(() => false)) {
        await medicineSearchInput.fill('cetirizine');
        await page.waitForTimeout(1000);
        let medOption = page.locator('div, li').filter({ hasText: new RegExp('Cetirizine', 'i') }).first();
