...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Dermatology', () => {

  test('TC_TRIAGE_DERMATOLOGY [VALID]: Receptionist Check-In & Triage for Dermatology', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.derma@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Dermatology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Dermatology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA derma' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA derma', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'dermatology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA derma" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Dermatology checkup and nurse triage evaluation');
