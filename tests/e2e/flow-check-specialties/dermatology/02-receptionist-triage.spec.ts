  ...
  test('TC_TRIAGE_DERMATOLOGY [VALID]: Receptionist Check-In & Triage for Dermatology', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        ...
        'qa.derma@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.derma@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.derma@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.derma@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA derma', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Dermatology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Dermatology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Dermatology checkup');
    }
