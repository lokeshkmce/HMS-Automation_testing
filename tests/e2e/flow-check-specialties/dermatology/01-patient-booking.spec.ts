...
test.describe('Step 1: Patient Appointment Booking - Dermatology', () => {
  ...
  test('TC_APPT_DERMATOLOGY [VALID]: Patient Appointment Booking for Dermatology', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Dermatology'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Dermatology', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Dermatology' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA derma', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'dermatology', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA derma" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA derma" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Dermatology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
