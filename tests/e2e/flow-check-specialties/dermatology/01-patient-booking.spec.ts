...
test.describe('Step 1: Patient Appointment Booking - Dermatology', () => {
  ...
  test('TC_APPT_DERMATOLOGY [VALID]: Patient Appointment Booking for Dermatology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA derma', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA derma' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dermatology' }))
      .first();
