...
test.describe('Step 3: Doctor Consultation & Prescription - Maternity', () => {

  test('TC_DOC_MATERNITY [VALID]: Doctor Consultation for Maternity', async ({ page }) => {
    ...
    if (await addPrescriptionBtn.isVisible({ timeout: 4000 }).catch(() => false)) {
      ...
      if (await medInput.isVisible({ timeout: 4000 }).catch(() => false)) {
        ...
        await medInput.fill('folic');
        await page.waitForTimeout(1000);

        const medOption = page.locator('div').filter({ hasText: new RegExp('^' + 'Folic Acid' + '$', 'i') })
