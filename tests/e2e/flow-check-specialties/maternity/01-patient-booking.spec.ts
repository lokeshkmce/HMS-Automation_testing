...
test.describe('Step 1: Patient Appointment Booking - Maternity', () => {
  ...
  test('TC_APPT_MATERNITY [VALID]: Patient Appointment Booking for Maternity', async ({ page }) => {
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA maternity', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'maternity', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA maternity" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA maternity" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
