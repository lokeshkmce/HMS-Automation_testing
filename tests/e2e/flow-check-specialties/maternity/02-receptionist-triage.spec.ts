...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Maternity', () => {
  ...
  test('TC_TRIAGE_MATERNITY [VALID]: Receptionist Check-In & Triage for Maternity', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.maternity@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Maternity', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Maternity/i }))
      .or(page.getByRole('option', { name: 'Dr. QA maternity' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA maternity', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'maternity', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA maternity" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Maternity checkup and nurse triage evaluation');
