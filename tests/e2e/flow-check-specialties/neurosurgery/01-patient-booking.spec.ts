...
test.describe('Step 1: Patient Appointment Booking - Neurosurgery', () => {
  ...
  test('TC_APPT_NEUROSURGERY [VALID]: Patient Appointment Booking for Neurosurgery', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA neurosurgery', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA neurosurgery' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Neurosurgery' }))
      .first();
