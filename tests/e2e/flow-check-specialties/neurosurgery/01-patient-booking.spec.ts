...
test.describe('Step 1: Patient Appointment Booking - Neurosurgery', () => {
  ...
  test('TC_APPT_NEUROSURGERY [VALID]: Patient Appointment Booking for Neurosurgery', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Neurosurgery'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Neurosurgery', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Neurosurgery' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA neurosurgery', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'neurosurgery', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA neurosurgery" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA neurosurgery" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Neurosurgery checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
