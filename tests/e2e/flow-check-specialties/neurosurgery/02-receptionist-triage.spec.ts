...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Neurosurgery', () => {
  ...
  test('TC_TRIAGE_NEUROSURGERY [VALID]: Receptionist Check-In & Triage for Neurosurgery', async ({ page }) => {
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA neurosurgery', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Neurosurgery', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Neurosurgery', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Neurosurgery checkup');
    }
