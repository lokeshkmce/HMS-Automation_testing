...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Neurosurgery', () => {
  ...
  test('TC_TRIAGE_NEUROSURGERY [VALID]: Receptionist Check-In & Triage for Neurosurgery', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.neurosurgery@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Neurosurgery', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Neurosurgery/i }))
      .or(page.getByRole('option', { name: 'Dr. QA neurosurgery' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA neurosurgery', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'neurosurgery', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA neurosurgery" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Neurosurgery checkup and nurse triage evaluation');
