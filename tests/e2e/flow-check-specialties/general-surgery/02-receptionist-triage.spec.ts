...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - General Surgery', () => {
  ...
  test('TC_TRIAGE_GENERAL_SURGERY [VALID]: Receptionist Check-In & Triage for General Surgery', async ({ page }) => {
    ...
    const docOption = page.getByRole('option', { name: 'Dr General Surgery', exact: true })
      .or(page.getByRole('option', { name: /Dr.*General Surgery/i }))
      .or(page.getByRole('option', { name: 'Dr. QA generalsurgery' }))
