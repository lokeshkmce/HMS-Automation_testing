...
test.describe('Step 1: Patient Appointment Booking - General Surgery', () => {
  ...
  test('TC_APPT_GENERAL_SURGERY [VALID]: Patient Appointment Booking for General Surgery', async ({ page }) => {
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA generalsurgery', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'general', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA generalsurgery" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA generalsurgery" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine General Surgery checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
