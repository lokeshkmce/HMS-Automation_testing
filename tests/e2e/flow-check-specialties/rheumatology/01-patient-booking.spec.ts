...
test.describe('Step 1: Patient Appointment Booking - Rheumatology', () => {
  ...
  test('TC_APPT_RHEUMATOLOGY [VALID]: Patient Appointment Booking for Rheumatology', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Rheumatology'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Rheumatology', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Rheumatology' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA rheumatology', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'rheumatology', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA rheumatology" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA rheumatology" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Rheumatology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
