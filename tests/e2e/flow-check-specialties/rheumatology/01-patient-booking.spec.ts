...
test.describe('Step 1: Patient Appointment Booking - Rheumatology', () => {
  ...
  test('TC_APPT_RHEUMATOLOGY [VALID]: Patient Appointment Booking for Rheumatology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA rheumatology', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA rheumatology' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Rheumatology' }))
      .first();
