...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Rheumatology', () => {

  test('TC_TRIAGE_RHEUMATOLOGY [VALID]: Receptionist Check-In & Triage for Rheumatology', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.rheumatology@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Rheumatology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Rheumatology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA rheumatology' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA rheumatology', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'rheumatology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA rheumatology" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Rheumatology checkup and nurse triage evaluation');
