...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Rheumatology', () => {
  ...
  test('TC_TRIAGE_RHEUMATOLOGY [VALID]: Receptionist Check-In & Triage for Rheumatology', async ({ page }) => {
    ...
    const emailsToTry = Array.from(new Set([
      'qa.rheumatology@omnivva.com',
      'qa.rheumatology@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
      'qa.rheumatology@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
      'qa.rheumatology@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
      'qa.rheumatology@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
    ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA rheumatology', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Rheumatology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Rheumatology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Rheumatology checkup');
    }
