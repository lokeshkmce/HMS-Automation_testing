  ...
  test('TC_TRIAGE_INTERNAL_MEDICINE [VALID]: Receptionist Check-In & Triage for Internal Medicine', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.internal.medicine@omnivva.com',
        'qa.internal.medicine@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.internal.medicine@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.internal.medicine@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.internal.medicine@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA internal.medicine', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Internal Medicine', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Internal Medicine', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Internal Medicine checkup');
    }
