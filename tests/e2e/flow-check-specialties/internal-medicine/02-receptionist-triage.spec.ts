...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Internal Medicine', () => {

  test('TC_TRIAGE_INTERNAL_MEDICINE [VALID]: Receptionist Check-In & Triage for Internal Medicine', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.internal.medicine@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Internal Medicine', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Internal Medicine/i }))
      .or(page.getByRole('option', { name: 'Dr. QA internal.medicine' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA internal.medicine', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'internal', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA internal.medicine" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Internal Medicine checkup and nurse triage evaluation');
