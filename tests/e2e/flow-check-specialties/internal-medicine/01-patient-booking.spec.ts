...
test.describe('Step 1: Patient Appointment Booking - Internal Medicine', () => {
  ...
  test('TC_APPT_INTERNAL_MEDICINE [VALID]: Patient Appointment Booking for Internal Medicine', async ({ page }) => {
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA internal.medicine', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'internal', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA internal.medicine" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA internal.medicine" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
