...
test.describe('Step 1: Patient Appointment Booking - Ophthalmology', () => {
  ...
  test('TC_APPT_OPHTHALMOLOGY [VALID]: Patient Appointment Booking for Ophthalmology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA opthalmology', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA opthalmology' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Ophthalmology' }))
      .first();
