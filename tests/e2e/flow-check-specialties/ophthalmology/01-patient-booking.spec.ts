...
test.describe('Step 1: Patient Appointment Booking - Ophthalmology', () => {
  ...
  test('TC_APPT_OPHTHALMOLOGY [VALID]: Patient Appointment Booking for Ophthalmology', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Ophthalmology'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Ophthalmology', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Ophthalmology' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA opthalmology', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'ophthalmology', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA opthalmology" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA opthalmology" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Ophthalmology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
