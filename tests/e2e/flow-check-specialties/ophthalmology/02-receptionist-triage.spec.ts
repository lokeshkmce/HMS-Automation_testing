...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Ophthalmology', () => {
  ...
  test('TC_TRIAGE_OPHTHALMOLOGY [VALID]: Receptionist Check-In & Triage for Ophthalmology', async ({ page }) => {
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA opthalmology', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Ophthalmology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Ophthalmology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Ophthalmology checkup');
    }
