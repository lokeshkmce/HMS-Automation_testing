...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Ophthalmology', () => {

  test('TC_TRIAGE_OPHTHALMOLOGY [VALID]: Receptionist Check-In & Triage for Ophthalmology', async ({ page }) => {
    ...
    const docOption = page.getByRole('option', { name: 'Dr Ophthalmology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Ophthalmology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA opthalmology' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA opthalmology', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'ophthalmology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA opthalmology" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Ophthalmology checkup and nurse triage evaluation');
