  ...
  test('TC_TRIAGE_PLASTIC_SURGERY [VALID]: Receptionist Check-In & Triage for Plastic Surgery', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.plastic.surgery@omnivva.com',
        'qa.plastic.surgery@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.plastic.surgery@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.plastic.surgery@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.plastic.surgery@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA plastic.surgery', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Plastic Surgery', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Plastic Surgery', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Plastic Surgery checkup');
    }
