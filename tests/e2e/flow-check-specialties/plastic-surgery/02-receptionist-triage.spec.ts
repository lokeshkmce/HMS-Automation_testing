...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Plastic Surgery', () => {
  ...
  test('TC_TRIAGE_PLASTIC_SURGERY [VALID]: Receptionist Check-In & Triage for Plastic Surgery', async ({ page }) => {
    ...
    const docOption = page.getByRole('option', { name: 'Dr Plastic Surgery', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Plastic Surgery/i }))
      .or(page.getByRole('option', { name: 'Dr. QA plastic.surgery' }))
