...
test.describe('Step 1: Patient Appointment Booking - Plastic Surgery', () => {
  ...
  test('TC_APPT_PLASTIC_SURGERY [VALID]: Patient Appointment Booking for Plastic Surgery', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Plastic Surgery'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Plastic Surgery', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Plastic Surgery' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA plastic.surgery', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'plastic', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA plastic.surgery" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA plastic.surgery" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Plastic Surgery checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
