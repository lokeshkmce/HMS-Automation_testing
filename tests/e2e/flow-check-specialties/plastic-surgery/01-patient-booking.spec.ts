...
test.describe('Step 1: Patient Appointment Booking - Plastic Surgery', () => {
  ...
  test('TC_APPT_PLASTIC_SURGERY [VALID]: Patient Appointment Booking for Plastic Surgery', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA plastic.surgery', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA plastic.surgery' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Plastic Surgery' }))
      .first();
