...
test.describe('Step 3: Doctor Consultation & Prescription - Cardio Surgery', () => {
  ...
  test('TC_DOC_CARDIO_SURGERY [VALID]: Doctor Consultation for Cardio Surgery', async ({ page }) => {
    ...
    if (await provisionalDiagnosisInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await provisionalDiagnosisInput.fill('Diagnosis - Cardio Surgery');
    }
    ...
    if (await writePrescriptionBtn.isVisible({ timeout: 4000 }).catch(() => false)) {
      ...
      if (await medicineSearchInput.isVisible({ timeout: 4000 }).catch(() => false)) {
        await medicineSearchInput.fill('aspirin');
        await page.waitForTimeout(1000);
        let medOption = page.locator('div, li').filter({ hasText: new RegExp('Aspirin', 'i') }).first();
