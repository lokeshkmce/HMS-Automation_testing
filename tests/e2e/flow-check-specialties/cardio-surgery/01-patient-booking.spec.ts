...
test.describe('Step 1: Patient Appointment Booking - Cardio Surgery', () => {
  ...
  test('TC_APPT_CARDIO_SURGERY [VALID]: Patient Appointment Booking for Cardio Surgery', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA cardiosurgery', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA cardiosurgery' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Cardio Surgery' }))
      .first();
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Cardio Surgery checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
