...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Cardio Surgery', () => {
  ...
  test('TC_TRIAGE_CARDIO_SURGERY [VALID]: Receptionist Check-In & Triage for Cardio Surgery', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.cardiosurgery@omnivva.com',
        'qa.cardiosurgery@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.cardiosurgery@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.cardiosurgery@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.cardiosurgery@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA cardiosurgery', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Cardio Surgery', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Cardio Surgery', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Cardio Surgery checkup');
    }
