...
test.describe('Step 3: Doctor Consultation & Prescription - Dental', () => {
  ...
  test('TC_DOC_DENTAL [VALID]: Doctor Consultation for Dental', async ({ page }) => {
    ...
    if (await provisionalDiagnosisInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await provisionalDiagnosisInput.fill('Diagnosis - Dental');
    }
    ...
    if (await writePrescriptionBtn.isVisible({ timeout: 4000 }).catch(() => false)) {
      ...
      if (await medicineSearchInput.isVisible({ timeout: 4000 }).catch(() => false)) {
        await medicineSearchInput.fill('amoxicillin');
        await page.waitForTimeout(1000);
        let medOption = page.locator('div, li').filter({ hasText: new RegExp('Amoxicillin', 'i') }).first();
