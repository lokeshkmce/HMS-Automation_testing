...
test.describe('Step 1: Patient Appointment Booking - Dental', () => {
  ...
  test('TC_APPT_DENTAL [VALID]: Patient Appointment Booking for Dental', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Dental'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Dental', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Dental' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA Dental', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'dental', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA Dental" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA Dental" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Dental checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
