...
test.describe('Step 1: Patient Appointment Booking - Dental', () => {
  ...
  test('TC_APPT_DENTAL [VALID]: Patient Appointment Booking for Dental', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA dental', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA dental' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dental' }))
      .first();
