...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Dental', () => {
  ...
  test('TC_TRIAGE_DENTAL [VALID]: Receptionist Check-In & Triage for Dental', async ({ page }) => {
    ...
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.dental@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr. QA Dental' })
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA Dental', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'dental', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA Dental" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Dental checkup and nurse triage evaluation');
