...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Dental', () => {
  ...
  test('TC_TRIAGE_DENTAL [VALID]: Receptionist Check-In & Triage for Dental', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        ...
        'qa.dental@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.dental@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.dental@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.dental@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA dental', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Dental', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Dental', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Dental checkup');
    }
