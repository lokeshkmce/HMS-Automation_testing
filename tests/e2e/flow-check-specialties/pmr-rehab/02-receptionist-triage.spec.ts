  ...
  test('TC_TRIAGE_PMR_REHAB [VALID]: Receptionist Check-In & Triage for PMR & Rehab', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.pmr.rehab@omnivva.com',
        'qa.pmr.rehab@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.pmr.rehab@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.pmr.rehab@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.pmr.rehab@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA pmr.rehab', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('PMR .* Rehab', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('PMR & Rehab', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine PMR & Rehab checkup');
    }
