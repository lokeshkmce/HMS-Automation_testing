...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - PMR & Rehab', () => {

  test('TC_TRIAGE_PMR_REHAB [VALID]: Receptionist Check-In & Triage for PMR & Rehab', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.pmr.rehab@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr PMR Rehab', exact: true })
      .or(page.getByRole('option', { name: /Dr.*PMR Rehab/i }))
      .or(page.getByRole('option', { name: 'Dr. QA pmr.rehab' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA pmr.rehab', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'pmr', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA pmr.rehab" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine PMR & Rehab checkup and nurse triage evaluation');
