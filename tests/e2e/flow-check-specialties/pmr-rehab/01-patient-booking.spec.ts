...
test.describe('Step 1: Patient Appointment Booking - PMR & Rehab', () => {
  ...
  test('TC_APPT_PMR_REHAB [VALID]: Patient Appointment Booking for PMR & Rehab', async ({ page }) => {
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine PMR & Rehab checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
