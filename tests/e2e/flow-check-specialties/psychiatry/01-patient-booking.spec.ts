...
test.describe('Step 1: Patient Appointment Booking - Psychiatry', () => {
  ...
  test('TC_APPT_PSYCHIATRY [VALID]: Patient Appointment Booking for Psychiatry', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA psychiatry', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA psychiatry' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Psychiatry' }))
      .first();
