...
test.describe('Step 1: Patient Appointment Booking - Psychiatry', () => {
  ...
  test('TC_APPT_PSYCHIATRY [VALID]: Patient Appointment Booking for Psychiatry', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Psychiatry'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Psychiatry', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Psychiatry' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA psychiatry', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'psychiatry', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA psychiatry" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA psychiatry" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Psychiatry checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
