...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Psychiatry', () => {
  ...
  test('TC_TRIAGE_PSYCHIATRY [VALID]: Receptionist Check-In & Triage for Psychiatry', async ({ page }) => {
    ...
    const docOption = page.getByRole('option', { name: 'Dr Psychiatry', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Psychiatry/i }))
      .or(page.getByRole('option', { name: 'Dr. QA psychiatry' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA psychiatry', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'psychiatry', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA psychiatry" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Psychiatry checkup and nurse triage evaluation');
