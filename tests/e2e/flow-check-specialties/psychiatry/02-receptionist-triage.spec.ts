...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Psychiatry', () => {
  ...
  test('TC_TRIAGE_PSYCHIATRY [VALID]: Receptionist Check-In & Triage for Psychiatry', async ({ page }) => {
    ...
    const emailsToTry = Array.from(new Set([
      ...
      'qa.psychiatry@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
      'qa.psychiatry@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
      'qa.psychiatry@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
      'qa.psychiatry@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
    ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA psychiatry', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Psychiatry', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Psychiatry', 'i') }))
        .first();
