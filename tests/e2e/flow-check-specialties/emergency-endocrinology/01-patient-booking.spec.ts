...
test.describe('Step 1: Patient Appointment Booking - Emergency & Endocrinology', () => {
  ...
  test('TC_APPT_EMERGENCY_ENDOCRINOLOGY [VALID]: Patient Appointment Booking for Emergency & Endocrinology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA emergency.endo', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA emergency.endo' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Emergency & Endocrinology' }))
      .first();
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Emergency & Endocrinology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
