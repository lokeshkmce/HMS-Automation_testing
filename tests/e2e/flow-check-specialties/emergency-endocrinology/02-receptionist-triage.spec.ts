...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Emergency & Endocrinology', () => {
  ...
  test('TC_TRIAGE_EMERGENCY_ENDOCRINOLOGY [VALID]: Receptionist Check-In & Triage for Emergency & Endocrinology', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.emergency.endo@ominvva.com',
        'qa.emergency.endo@ominvva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.emergency.endo@ominvva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.emergency.endo@ominvva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.emergency.endo@ominvva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA emergency.endo', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Emergency .* Endocrinology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Emergency & Endocrinology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Emergency & Endocrinology checkup');
    }
