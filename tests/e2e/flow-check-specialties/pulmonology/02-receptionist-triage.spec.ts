...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Pulmonology', () => {

  test('TC_TRIAGE_PULMONOLOGY [VALID]: Receptionist Check-In & Triage for Pulmonology', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.pulmonology@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Pulmonology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Pulmonology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA pulmonology' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA pulmonology', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'pulmonology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA pulmonology" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Pulmonology checkup and nurse triage evaluation');
