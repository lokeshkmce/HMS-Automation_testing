...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Pulmonology', () => {
  ...
  test('TC_TRIAGE_PULMONOLOGY [VALID]: Receptionist Check-In & Triage for Pulmonology', async ({ page }) => {
    ...
    const emailsToTry = Array.from(new Set([
      'qa.pulmonology@omnivva.com',
      'qa.pulmonology@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
      'qa.pulmonology@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
      'qa.pulmonology@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
      'qa.pulmonology@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
    ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA pulmonology', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Pulmonology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Pulmonology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Pulmonology checkup');
    }
