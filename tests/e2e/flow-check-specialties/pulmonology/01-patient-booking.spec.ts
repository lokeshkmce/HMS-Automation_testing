...
test.describe('Step 1: Patient Appointment Booking - Pulmonology', () => {
  ...
  test('TC_APPT_PULMONOLOGY [VALID]: Patient Appointment Booking for Pulmonology', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Pulmonology'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Pulmonology', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Pulmonology' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA pulmonology', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'pulmonology', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA pulmonology" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA pulmonology" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Pulmonology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
