...
test.describe('Step 1: Patient Appointment Booking - Pulmonology', () => {
  ...
  test('TC_APPT_PULMONOLOGY [VALID]: Patient Appointment Booking for Pulmonology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA pulmonology', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA pulmonology' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Pulmonology' }))
      .first();
