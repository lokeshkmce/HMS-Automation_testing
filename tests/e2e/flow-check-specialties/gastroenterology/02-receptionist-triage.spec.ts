...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Gastroenterology', () => {

  test('TC_TRIAGE_GASTROENTEROLOGY [VALID]: Receptionist Check-In & Triage for Gastroenterology', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.gastro@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Gastroenterology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Gastroenterology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA gastro' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA gastro', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'gastroenterology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA gastro" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Gastroenterology checkup and nurse triage evaluation');
