  ...
  test('TC_TRIAGE_GASTROENTEROLOGY [VALID]: Receptionist Check-In & Triage for Gastroenterology', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.gastro@ominvva.com',
        'qa.gastro@ominvva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.gastro@ominvva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.gastro@ominvva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.gastro@ominvva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA gastro', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Gastroenterology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Gastroenterology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Gastroenterology checkup');
    }
