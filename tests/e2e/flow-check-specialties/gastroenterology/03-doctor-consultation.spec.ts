...
test.describe('Step 3: Doctor Consultation & Prescription - Gastroenterology', () => {
  ...
  test('TC_DOC_GASTROENTEROLOGY [VALID]: Doctor Consultation for Gastroenterology', async ({ page }) => {
    ...
    if (await provisionalDiagnosisInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await provisionalDiagnosisInput.fill('Diagnosis - Gastroenterology');
    }
    ...
    if (await writePrescriptionBtn.isVisible({ timeout: 4000 }).catch(() => false)) {
      ...
      if (await medicineSearchInput.isVisible({ timeout: 4000 }).catch(() => false)) {
        await medicineSearchInput.fill('pantoprazole');
        await page.waitForTimeout(1000);
        let medOption = page.locator('div, li').filter({ hasText: new RegExp('Pantoprazole', 'i') }).first();
