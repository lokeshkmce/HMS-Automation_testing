...
test.describe('Step 1: Patient Appointment Booking - Gastroenterology', () => {
  ...
  test('TC_APPT_GASTROENTEROLOGY [VALID]: Patient Appointment Booking for Gastroenterology', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Gastroenterology'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Gastroenterology', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Gastroenterology' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA gastro', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'gastroenterology', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA gastro" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA gastro" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Gastroenterology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
