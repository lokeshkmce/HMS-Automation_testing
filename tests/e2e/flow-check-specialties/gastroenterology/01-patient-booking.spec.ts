...
test.describe('Step 1: Patient Appointment Booking - Gastroenterology', () => {
  ...
  test('TC_APPT_GASTROENTEROLOGY [VALID]: Patient Appointment Booking for Gastroenterology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA gastro', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA gastro' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Gastroenterology' }))
      .first();
