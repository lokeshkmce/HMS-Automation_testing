...
test.describe('Step 1: Patient Appointment Booking - Orthopedics', () => {
  ...
  test('TC_APPT_ORTHOPEDICS [VALID]: Patient Appointment Booking for Orthopedics', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Orthopedics'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Orthopedics', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Orthopedics' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA orthopedics', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'orthopedics', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA orthopedics" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA orthopedics" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Orthopedics checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
