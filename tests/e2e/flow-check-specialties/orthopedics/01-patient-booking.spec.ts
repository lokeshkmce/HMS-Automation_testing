...
test.describe('Step 1: Patient Appointment Booking - Orthopedics', () => {
  ...
  test('TC_APPT_ORTHOPEDICS [VALID]: Patient Appointment Booking for Orthopedics', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA orthopedics', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA orthopedics' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Orthopedics' }))
      .first();
