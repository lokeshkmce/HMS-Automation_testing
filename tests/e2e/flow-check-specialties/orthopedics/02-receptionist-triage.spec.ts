...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Orthopedics', () => {

  test('TC_TRIAGE_ORTHOPEDICS [VALID]: Receptionist Check-In & Triage for Orthopedics', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.orthopedics@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Orthopedics', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Orthopedics/i }))
      .or(page.getByRole('option', { name: 'Dr. QA orthopedics' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA orthopedics', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'orthopedics', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA orthopedics" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Orthopedics checkup and nurse triage evaluation');
