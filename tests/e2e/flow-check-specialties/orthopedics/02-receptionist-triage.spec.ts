  ...
  test('TC_TRIAGE_ORTHOPEDICS [VALID]: Receptionist Check-In & Triage for Orthopedics', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.orthopedics@omnivva.com',
        'qa.orthopedics@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.orthopedics@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.orthopedics@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.orthopedics@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA orthopedics', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Orthopedics', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Orthopedics', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Orthopedics checkup');
    }
