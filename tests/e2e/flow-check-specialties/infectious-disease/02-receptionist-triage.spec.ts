  ...
  test('TC_TRIAGE_INFECTIOUS_DISEASE [VALID]: Receptionist Check-In & Triage for Infectious Disease', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.infectious.disease@omnivva.com',
        'qa.infectious.disease@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.infectious.disease@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.infectious.disease@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.infectious.disease@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA infectious.disease', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Infectious Disease', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Infectious Disease', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Infectious Disease checkup');
    }
