...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Infectious Disease', () => {

  test('TC_TRIAGE_INFECTIOUS_DISEASE [VALID]: Receptionist Check-In & Triage for Infectious Disease', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.infectious.disease@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Infectious Disease', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Infectious Disease/i }))
      .or(page.getByRole('option', { name: 'Dr. QA infectious.disease' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA infectious.disease', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'infectious', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA infectious.disease" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Infectious Disease checkup and nurse triage evaluation');
