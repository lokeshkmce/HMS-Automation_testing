...
test.describe('Step 1: Patient Appointment Booking - Infectious Disease', () => {
  ...
  test('TC_APPT_INFECTIOUS_DISEASE [VALID]: Patient Appointment Booking for Infectious Disease', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA infectious.disease', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA infectious.disease' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Infectious Disease' }))
      .first();
