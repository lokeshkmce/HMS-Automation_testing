...
test.describe('Step 1: Patient Appointment Booking - Infectious Disease', () => {
  ...
  test('TC_APPT_INFECTIOUS_DISEASE [VALID]: Patient Appointment Booking for Infectious Disease', async ({ page }) => {
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA infectious.disease', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'infectious', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA infectious.disease" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA infectious.disease" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Infectious Disease checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
