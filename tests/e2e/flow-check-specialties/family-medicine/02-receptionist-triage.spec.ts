...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Family Medicine', () => {
  ...
  test('TC_TRIAGE_FAMILY_MEDICINE [VALID]: Receptionist Check-In & Triage for Family Medicine', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.family@ominvva.com',
        'qa.family@ominvva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.family@ominvva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.family@ominvva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.family@ominvva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA family', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Family Medicine', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Family Medicine', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Family Medicine checkup');
    }
