...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Family Medicine', () => {
  ...
  test('TC_TRIAGE_FAMILY_MEDICINE [VALID]: Receptionist Check-In & Triage for Family Medicine', async ({ page }) => {
    ...
    const docOption = page.getByRole('option', { name: 'Dr Family Medicine', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Family Medicine/i }))
      .or(page.getByRole('option', { name: 'Dr. QA family' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA family', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'family', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA family" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Family Medicine checkup and nurse triage evaluation');
