...
test.describe('Step 1: Patient Appointment Booking - Family Medicine', () => {
  ...
  test('TC_APPT_FAMILY_MEDICINE [VALID]: Patient Appointment Booking for Family Medicine', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Family Medicine'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Family Medicine', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Family Medicine' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA family', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'family', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA family" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA family" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Family Medicine checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
