...
test.describe('Step 1: Patient Appointment Booking - Family Medicine', () => {
  ...
  test('TC_APPT_FAMILY_MEDICINE [VALID]: Patient Appointment Booking for Family Medicine', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA family', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA family' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Family Medicine' }))
      .first();
