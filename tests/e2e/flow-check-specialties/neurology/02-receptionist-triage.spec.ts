...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Neurology', () => {

  test('TC_TRIAGE_NEUROLOGY [VALID]: Receptionist Check-In & Triage for Neurology', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.neurology@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Neurology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Neurology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA neurology' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA neurology', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'neurology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA neurology" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Neurology checkup and nurse triage evaluation');
