  ...
  test('TC_TRIAGE_NEUROLOGY [VALID]: Receptionist Check-In & Triage for Neurology', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.neurology@omnivva.com',
        'qa.neurology@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.neurology@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.neurology@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.neurology@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA neurology', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Neurology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Neurology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Neurology checkup');
    }
