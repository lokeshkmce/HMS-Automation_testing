...
test.describe('Step 1: Patient Appointment Booking - Neurology', () => {
  ...
  test('TC_APPT_NEUROLOGY [VALID]: Patient Appointment Booking for Neurology', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Neurology'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Neurology', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Neurology' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA neurology', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'neurology', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA neurology" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA neurology" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Neurology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
