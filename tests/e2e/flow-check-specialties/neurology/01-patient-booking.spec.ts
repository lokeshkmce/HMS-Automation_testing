...
test.describe('Step 1: Patient Appointment Booking - Neurology', () => {
  ...
  test('TC_APPT_NEUROLOGY [VALID]: Patient Appointment Booking for Neurology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA neurology', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA neurology' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Neurology' }))
      .first();
