...
test.describe('Step 1: Patient Appointment Booking - Nephrology', () => {
  ...
  test('TC_APPT_NEPHROLOGY [VALID]: Patient Appointment Booking for Nephrology', async ({ page }) => {
    ...
    for (const facName of facilityList) {
      ...
      const firstWord = 'Nephrology'.split(' ')[0];
      const targetOption = page.getByRole('option', { name: 'Nephrology', exact: true })
        .or(page.getByRole('option', { name: new RegExp('^' + 'Nephrology' + '$', 'i') }))
    ...
    const targetDocText = page.getByText(new RegExp('Dr\\. QA nephrology', 'i'))
      .or(page.getByText(new RegExp('Dr\\.\\s*QA\\s*' + 'nephrology', 'i')))
      .first();

    await expect(targetDocText, `Target QA Doctor "Dr. QA nephrology" must be visible on Doctor Selection page`).toBeVisible({ timeout: 15_000 });
    ...
    if (!slotFound) {
      throw new Error(`Process Stopped: Target QA Doctor "Dr. QA nephrology" has 0 available slots! Skipping booking for non-QA doctors as requested.`);
    }
    ...
    if (await symptomsInput.isVisible({ timeout: 5000 }).catch(() => false)) {
      await symptomsInput.fill('Routine Nephrology checkup');
    }
    await page.getByRole('button', { name: 'Next' }).click();
