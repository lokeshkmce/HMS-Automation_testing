...
test.describe('Step 1: Patient Appointment Booking - Nephrology', () => {
  ...
  test('TC_APPT_NEPHROLOGY [VALID]: Patient Appointment Booking for Nephrology', async ({ page }) => {
    ...
    const targetDocCard = page.getByText('Dr. QA nephrology', { exact: false })
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Dr. QA nephrology' }))
      .or(page.locator('div, .MuiCard-root, .MuiPaper-root').filter({ hasText: 'Nephrology' }))
      .first();
