  ...
  test('TC_TRIAGE_NEPHROLOGY [VALID]: Receptionist Check-In & Triage for Nephrology', async ({ page }) => {
    ...
    if (await usernameInput.isVisible({ timeout: 10_000 }).catch(() => false)) {
      ...
      const emailsToTry = Array.from(new Set([
        'qa.nephrology@omnivva.com',
        'qa.nephrology@omnivva.com'.replace('cardio.surgery', 'cardiosurgery'),
        'qa.nephrology@omnivva.com'.replace('cardiosurgery', 'cardio.surgery'),
        'qa.nephrology@omnivva.com'.replace('@ominvva.com', '@omnivva.com'),
        'qa.nephrology@omnivva.com'.replace('@omnivva.com', '@ominvva.com')
      ]));
    ...
    if (await doctorFilter.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const targetDocOption = page.getByText('Dr. QA nephrology', { exact: false })
        .or(page.getByRole('option', { name: new RegExp('Nephrology', 'i') }))
        .or(page.locator('li[role="option"]').filter({ hasText: new RegExp('Nephrology', 'i') }))
        .first();
    ...
    if (await chiefComplaintInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      await chiefComplaintInput.fill('Routine Nephrology checkup');
    }
