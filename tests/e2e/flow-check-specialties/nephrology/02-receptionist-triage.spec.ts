...
test.describe('Step 2: Receptionist Check-In & Nurse Triage - Nephrology', () => {

  test('TC_TRIAGE_NEPHROLOGY [VALID]: Receptionist Check-In & Triage for Nephrology', async ({ page }) => {
    test.setTimeout(120_000);
    await page.goto('https://dev-hms.srivyn.in/');

    await page.getByRole('button', { name: 'Staff Login' }).click();
    await page.getByRole('textbox', { name: 'Username or Email' }).fill('qa.nephrology@omnivva.com');
    ...
    const docOption = page.getByRole('option', { name: 'Dr Nephrology', exact: true })
      .or(page.getByRole('option', { name: /Dr.*Nephrology/i }))
      .or(page.getByRole('option', { name: 'Dr. QA nephrology' }))
      .or(page.getByRole('option', { name: new RegExp('Dr\\. QA nephrology', 'i') }))
      .or(page.getByRole('option', { name: new RegExp('QA\\s*' + 'nephrology', 'i') }))
      .first();

    await expect(docOption, `QA Doctor "Dr. QA nephrology" must be visible in dropdown`).toBeVisible({ timeout: 15_000 });
    ...
    await complaintInput.fill('Routine Nephrology checkup and nurse triage evaluation');
